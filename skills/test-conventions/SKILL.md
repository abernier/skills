...
If you skip it, say so where the feature's decisions live — its ADR when there is one — with what adding it later would need. An unrecorded gap gets argued about again.
...
One minimal end-to-end test file with pinned assertions beats a layered test architecture. No oracle machinery, no multi-layer suite, unless it was asked for. Don't constrain too early: too many tests, types or abstractions ossify a system before its shape is known.
