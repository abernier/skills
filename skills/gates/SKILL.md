...
**Lowercase is the fast suite** — typically lint, format, types, build, unit tests, plus whatever else the repo gates on. Run it before every commit and fix what it reports.
...
Same commands, same versions, same configuration in development, in CI and in production. A check that exists only in CI, or a behaviour that appears only in production, is a bug found at the worst possible moment.
...
One PR lands as one commit on the main branch, so its history stays a list of changes rather than a replay of how it was reached.

The exception is a release bot's PR — a changesets version PR and its like. Squashing rewrites the bot's author and message, and the publish step's detection silently fails. Rebase those.
...
**Not automatic.** A step runs on every future comparison, and a gesture an existing mark already covers does not need one. Weigh it; if you skip it, say so where the feature's decisions live.
