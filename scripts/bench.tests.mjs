/**
 * The vitest plugin that runs this package's conformance check in a consumer's
 * own `vitest run`.
 *
...
const CONFORMANCE_FILE = fileURLToPath(
  new URL("./bench.conformance.test.ts", import.meta.url),
);

/**
 * Run the bench harness's conformance check as part of the repo's own
 * `vitest run`.
 *
 * ```ts
 * // vitest.config.ts
 ...
 * ```
 *
 * `bench.conformance.test.ts` is the one file only a consumer can run: it
 * derives its subject from `sourceRoots` at run time, and goes red when the
 * source tree moved and `bench.json` did not. This package's own tests stay
 * here and never ship.
 *
