 ...
 */
export declare function tracerbenchConfig(options: TracerbenchOptions): PlaywrightTestConfig;
/**
 ...
 * @returns {PlaywrightTestConfig}
 */
export declare function profilerConfig(options: ProfilerOptions): PlaywrightTestConfig;
export type TracerbenchServer = {
    /**
    ...
    /**
     * The `vite preview` flags for this run — `--outDir` (only when `TB_DIST` is set), `--port` and `--strictPort`, ready to append to whatever runs vite in your repo. A non-vite server ignores it and reads `port` and `dist` directly.
     */
    previewArgs: string;
};
export type TracerbenchOptions = {
    /**
    ...
    timeout?: number;
    /**
     * Shallow-merged over the derived `webServer`, for a server story the two knobs above do not cover — extra `env`, a different boot `timeout`, `stdout`.
     */
    webServer?: Partial<NonNullable<PlaywrightTestConfig["webServer"]>>;
};
export type ProfilerServer = {
    /**
